...
it('adds private deployment cookies to every Platform request without exposing them in account state', async () => {
  ...
  // Profile and balance use independent concurrent requests.
  expect(paths.slice(2, 4).toSorted()).toEqual(['/api/v0/users/get_user_summary', '/auth-api/v0/users/current'])
  expect(paths.slice(4)).toEqual(['/auth-api/v0/users/logout'])
