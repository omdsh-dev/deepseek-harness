...
工具调用在对话中显示为卡片：一个根调用树带其嵌套子调用，每个原子调用由所属视图渲染。用户看到运行中、成功、失败与中断状态，这些状态只来自冻结的调用/结果切片，并可通过宿主回调打开文件或检查调用。通用行与 Bash 行会为四种生命周期状态全部暴露本地化文本，而不依赖扫光、颜色或状态点；每个可展开行还通过 `aria-controls` 指向稳定的受控面板。Chat owner 会另行播报根调用的状态跃迁，但不会重放历史或讲述嵌套 dispatch 噪声。
