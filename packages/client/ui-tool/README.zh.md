...
工具调用在对话中显示为卡片：一个根调用树带其嵌套子调用，每个原子调用由所属视图渲染。用户看到运行中、成功、失败与中断状态，这些状态只来自冻结的调用/结果切片，并可通过宿主回调打开文件或检查调用。可展开的文件行向辅助技术暴露两个平级控件：一个带名称且承载 `aria-expanded` 的折叠按钮，以及已本地化的“打开文件 {path}”按钮；指针用户仍可从普通行表面展开。
