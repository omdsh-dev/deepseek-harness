...
function CatalogDropdown({
  ...
  const [tabStopId, setTabStopId] = useState<SessionId | null>(() => (
    currentSessionId
    ?? catalog?.entries.find((entry): entry is Extract<CatalogEntry, { kind: 'child' }> => entry.kind === 'child')?.id
    ?? null
  ))
  ...
  // Keep exactly one visible, enabled tree row in the page Tab sequence even
  // when refreshes or branch collapse remove the row that last held focus.
  useEffect(() => {
    if (!open) return
    ...
    >
      ...
      <button
        ...
        onKeyDown={(event) => {
          ...
          queueMicrotask(() => { focusAt(event.key === 'ArrowDown' ? 0 : treeItems(menuRef.current).length - 1) })
        }}
      >
        {variant === 'switcher'
