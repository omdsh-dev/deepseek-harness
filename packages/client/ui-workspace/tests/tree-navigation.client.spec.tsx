...
describe('Workspace tree keyboard owner', () => {
  ...
  it('ignores child controls and detached targets while pointer and focus promote rows', () => {
    ...
    latestNavigation.onFocusCapture({ target: detached })
    latestNavigation.onFocusCapture({ target: detachedRow })
    latestNavigation.onFocusCapture({ target: document })
    const keyEvent = { key: 'Home', altKey: false, ctrlKey: false, metaKey: false, preventDefault: vi.fn() }
    latestNavigation.onKeyDown({ ...keyEvent, target: document })
    fireEvent.keyDown(screen.getByTestId('tree'), { key: 'Home' })

    view.rerender(<Harness show={false} />)
    latestNavigation.onFocusCapture({ target: detached })
    latestNavigation.onPointerDownCapture({ target: detached })
    latestNavigation.onKeyDown({ ...keyEvent, target: detached })
