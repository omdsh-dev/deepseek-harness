...
分组、平铺与搜索呈现都会暴露一棵单选树。焦点进入树时，当前选中的 Session 优先成为唯一的顺序 Tab 入口；否则始终只有一行可进入 Tab 顺序。用上、下方向键在可见行之间移动，用 Home 与 End 到达边界，用右方向键展开 Workspace 或移动到其第一个子项，用左方向键折叠或返回父项，用 Enter 或空格激活行。输入可打印字符会按标签前缀查找，重复输入同一字符则循环匹配。指针或焦点交互会把对应行提升为活动树入口。活动行存在操作时，Tab 会进入这些操作；无需指针悬浮，也会保持可见的焦点指示。
...
折叠搜索是视图和添加操作旁的一枚区头按钮：激活后输入框会扩展并占据区头。非空白查询会以单一扁平结果列表替代任一浏览模式——不区分大小写的标题和 Workspace 子串匹配项会立即显示，经 250 ms 防抖的 Host 请求则会加入经过排序的当前对话内容匹配项及其摘要片段。每次新查询都会中止前一个请求；内容搜索失败时，元数据匹配项仍会显示，同时给出警告。列表最多显示 20 条结果，打开所选 Session 时不会清除查询。按 Escape 或使用清除操作会折叠输入框、把它移出无障碍树和 Tab 顺序，并把焦点恢复到持久的搜索按钮；如果用户已经把焦点移到别处，指针关闭会保留该焦点。
...
目录流或接纳失败时，系统会打开一个以可见警报作为描述的对话框，并把初始焦点放在非破坏性的“取消”操作上；关闭对话框或用**重新选择**启动新流程时，焦点会恢复到仍连接的选择器触发控件。
