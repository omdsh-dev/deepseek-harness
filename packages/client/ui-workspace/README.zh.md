...
分组、扁平和搜索结果视图都是只有一个顺序入口的树形复合控件。Tab 到达当前行；上／下或 Home／End 在行间移动；Enter 与空格激活行。在分组视图中，右方向键会打开已折叠的 Workspace，或进入它的第一条 Session；左方向键会关闭已打开的 Workspace，或从 Session 返回其 Workspace 行。只有当前行拥有的操作才参与顺序 Tab 导航。
...
搜索折叠时，输入框不出现在无障碍树和顺序导航中。Escape 或显式的清除操作会关闭搜索，并把焦点还给始终存在的搜索按钮；使用外部指针关闭时，则把焦点留在用户已经移到的位置。

### 管理会话

Session 行内的 Rename 操作打开一个以该行显示标题预填的对话框；确认未修改的标题是有意允许的——这正是把当前自动标题钉住、不再被重新生成覆盖的手势。Workspace Rename 的初始焦点进入已选中的名称输入框，Session Rename 也采用相同行为。行菜单交接会把持久存在的行操作按钮记录为弹窗打开控件，因此取消、失败后再取消以及成功重命名都会在该行仍存在时恢复焦点。Archive 不经确认对话框直接提交，归档集合回声落地后，该行从所有分组视图中消失。Fork 在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话。Workspace 行内的 Delete 操作会打开确认框，说明保留边界，并把初始焦点放在非破坏性的关闭操作上；取消会回到行操作按钮，成功后该分组被移除，其 Session 则留在 Ungrouped 下。
