...
A single-select question exposes a named, one-entry radio group: Arrow keys wrap through choices, Home and End move to an edge, and arrow movement selects without unexpectedly advancing the page. Activating a choice retains the immediate-advance workflow, then focuses the next question's selected radio, first checkbox, or custom field; pager moves and validation recovery use the same focus contract. Recommendation and option-description text is attached to each radio as its accessible description. A multi-select draft keeps its selected labels while the user opens or edits the named custom-answer field, so its submitted item may carry both `selected` and `custom`; a single-select custom answer remains exclusive. Question detail reuses the assistant-output `MarkdownText` primitive, including its GFM rendering and untrusted-content policy. The capped card keeps its title, navigation, and submission actions fixed while long detail and choices share an internal scroll region. "Skip this question" retains other drafts and emits the existing blank `{ selected: [] }` result for that item, while close rejects the whole wait as `ASK_CANCELLED`.
