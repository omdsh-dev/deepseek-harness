...
框架提供应用标题、Sidebar 导航、main 地标和具名的右侧栏。隐藏的右侧内容进入 inert 状态。键盘分隔条暴露所控制面板及像素范围；方向键调整宽度，Home/End 选择边界，Enter 切换左侧轨道或恢复右侧默认宽度。在不超过 600px 的视口中，栏宽过渡被禁用，使高倍缩放回流时的聚焦控件保持可见。
