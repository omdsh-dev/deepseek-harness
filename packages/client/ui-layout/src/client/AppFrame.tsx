...
import { CENTER_MIN, clampWidth, computeColumns, RIGHTBAR_DEFAULT_RATIO, RIGHTBAR_MAX_RATIO, RIGHTBAR_MIN, SIDEBAR_AUTO_COLLAPSE, SIDEBAR_COLLAPSED, SIDEBAR_DEFAULT, SIDEBAR_MAX, SIDEBAR_MIN } from './columns.ts'
  ...
  return (
    ...
    >
      ...
      <DragHandle side="sidebar" left={cols.sidebar} label={sidebarLabel}
        value={cols.sidebar} min={collapsedWidth} max={SIDEBAR_MAX} collapsed={sidebarCollapsed}
        valueText={sidebarCollapsed ? t('layout.collapsed') : t('layout.widthPixels', { value: cols.sidebar })}
        onSet={(width) => {
          if (width <= collapsedWidth) { if (!sidebarCollapsed) actions.toggleSidebar(); return }
