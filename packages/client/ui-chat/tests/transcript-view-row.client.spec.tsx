...
describe('TranscriptViewRow', () => {
  ...
  it.each([
    ...
    expect(screen.queryByRole('menuitemradio', { name: 'Normal' })).toBeNull()
    fireEvent.click(screen.getByRole('menuitemradio', { name: label }))
    expect(b.setTranscriptView).toHaveBeenCalledWith(mode)
    const trigger = screen.getByRole('button', { name: `Work details: ${label}` })
    fireEvent.click(trigger)
    expect(screen.getByRole('menuitemradio', { name: 'Compact' })).toBeDefined()
  ...
  it('shows all three work-detail values in Chinese', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '工作过程展示: 简洁' }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: '详细' }))
    expect(b.setTranscriptView).toHaveBeenLastCalledWith('detailed')
    fireEvent.click(screen.getByRole('button', { name: '工作过程展示: 详细' }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: '完全展开' }))
    expect(b.setTranscriptView).toHaveBeenLastCalledWith('expanded')
    expect(screen.getByRole('button', { name: '工作过程展示: 完全展开' })).toBeDefined()
  })
  ...
  it('returns focus before publishing a mode change without refocusing after the menu closes', async () => {
    ...
    fireEvent.click(trigger)
    const item = screen.getByRole('menuitemradio', { name: 'Detailed' })
    item.focus()
...
describe('LinkOpeningRow', () => {
  it('follows browser availability without clearing the saved destination', () => {
    ...
    expect(screen.getByRole('button', { name: 'Open chat links in: New browser tab' })).toBeDefined()
  ...
  it.each([
    ...
    fireEvent.click(screen.getByRole('button', { name: `${title}: ${sidebar}` }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: newTab }))
    expect(setLinkOpening).toHaveBeenLastCalledWith('new-tab')
    fireEvent.click(screen.getByRole('button', { name: `${title}: ${newTab}` }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: sidebar }))
    expect(setLinkOpening).toHaveBeenLastCalledWith('sidebar')
    expect(screen.getByRole('button', { name: `${title}: ${sidebar}` }).getAttribute('aria-expanded')).toBe('false')
...
describe('PerformanceUsageRow', () => {
  it('selects compact statistics independently of conversation display', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Performance & usage: Detailed' }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: 'Compact' }))
    expect(setPerformanceUsage).toHaveBeenCalledWith('compact')
    expect(screen.getByRole('button', { name: 'Performance & usage: Compact' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Work details: Compact' })).toBeTruthy()
    expect(b.setTranscriptView).not.toHaveBeenCalled()
  })
})
