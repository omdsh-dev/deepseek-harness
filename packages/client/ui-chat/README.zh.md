...
对话记录暴露一个具名且关闭实时更新的 `log`，durable 与 pending 人工消息、可见 Assistant 回复分别暴露具名 `article` 边界。因此流式文本仍可直接阅读，但不会造成逐 token 实时区域播报。只有已观测到运行中的 Session 转为空闲时，才发出一次视觉隐藏且原子的状态通知；打开已完成历史不播报。
