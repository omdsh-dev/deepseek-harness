...
选择、时间线导航、折叠、搜索与请求汇总只覆盖已加载的窗口。ledger 只保留一个顺序行入口；上／下方向键逐条移动逻辑记录，Home／End 移到已加载窗口的边界，导航会先把虚拟化目标滚入视图再聚焦。Enter 与空格激活聚焦记录或折叠摘要。只有活动行内嵌的 Request 操作会进入顺序导航。选择记录会打开局部检查器，查看 token 用量、耗时、输入、输出、计时与持久图片。其详情标签只保留一个顺序 Tab 入口；左／右方向键在可用详情间循环，Home／End 移到边界，同时激活并聚焦目标。图片 URL 使用 Conversation 拥有的逐会话缓存，因此 Chat 与 Trajectory 对每个附件共享一次已授权读取。没有文本的记录会用图片数量标记其行。独立运行的压缩请求会按时间顺序显示在自己的 `Between turns` 区段中，而带编号的压缩仍位于其所属轮次内。

### 时间概览

固定在记录表上方的 Overview 区域从左到右投影记录的真实开始时间与耗时；助手时间条区分记录到的 TTFT 与解码时间，悬停 500 ms 后可查看精确时刻与耗时详情。该概览同时是多选 listbox：方向键浏览记录，Shift 加方向键扩展闭合聚焦范围，Home／End 移到边界，Enter 或空格打开活动记录，Control 或 Command 加 A 聚焦全部记录，Escape 清除范围。指针拖选会把记录表聚焦到所选区间内任何时刻处于活动的记录；滚轮手势用于缩放时间域；右键单击会清除所选区间，在已放大的视口上按住右键拖动则会平移。初始视图与流式更新都停留在尾部；向上滚动会暂停跟随，因此新记录不会打断对旧记录的检查。
