...
Users reach the shell through the sidebar's bottom Settings control; the compact rail retains the control's accessible name. Feature plugins contribute their pages and onboarding steps through the slot ledgers this shell projects. At narrow viewports, the section navigation stacks above the active page, contributed rows reflow vertically, and keyboard focus is scrolled into the visible modal viewport. After a Host connection failure, a pale-yellow **Disconnected** action appears to the right of Settings. Automatic recovery shows **Connecting** with one to three dots advancing every 500ms. Hover or keyboard focus changes either yellow label to **Reconnect now** without changing its background; press feedback stays within the warning palette, and selecting it starts retry 1 immediately. Recovery changes the region to pale-green **Connected** for two seconds before it disappears. The icon, left-aligned text origin, height, and width remain fixed across every visible state. Initial startup and uninterrupted healthy operation remain silent. The shell renders the modal panel, the navigation built from `settings.section` entries, and exactly one mounted onboarding step at a time.
