...
When more than one View is registered, the Session header exposes one named tab list with only the selected tab in sequential navigation. Left and Right wrap across Views, while Home and End choose an edge; each key both selects and focuses the target View. Stable per-Session tab and panel ids preserve `aria-controls` and `aria-labelledby` ownership across header and body slots, and the active panel remains the next sequential focus destination.
