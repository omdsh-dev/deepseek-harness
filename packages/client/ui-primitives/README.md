...
`Button` forwards its native element ref for interaction owners. `Modal` makes the application root inert while any shared dialog is open, keeps only the top dialog interactive, chooses and contains focus, limits Escape and mask dismissal to that top dialog, and restores focus to an explicit connected target or the connected opening control.
