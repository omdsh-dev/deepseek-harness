...
`Button` forwards its native element ref for interaction owners. `Menu` turns its inline anchor into a named menu button, or synchronizes the popup relationship through an explicitly referenced external trigger; it moves focus into enabled items, owns arrow, Home, End, typeahead, submenu, Escape, and Tab operation, and restores focus when the menu closes. Its item contract separates actions from radio and checkbox choices: checked choices expose `menuitemradio` or `menuitemcheckbox` with synchronized `aria-checked`, while their visual check mark stays out of the accessible name. `DisclosureRow` gives either its whole-row control or leading button a stable `aria-controls` target; the empty hidden panel remains addressable while collapsed, but its feature-owned body stays unmounted. When collapsed content has its own action, a separate named leading button owns disclosure while the plain row remains a pointer target, so no native control is nested inside a pseudo-button. `Modal` makes the application root inert while any shared dialog is open, keeps only the top dialog interactive, chooses and contains focus, limits Escape and mask dismissal to that top dialog, and restores focus to an explicit connected target or the connected opening control. `RiskConfirmation` adds an associated risk description, a decorative warning glyph, and required-acknowledgement initial focus; its consumers remain responsible for supplying a durable restoration target when the invoking menu row is transient.

### Rendering agent output

`MarkdownText` renders untrusted GFM and TeX math, blocks unsafe links and images, and can turn resolved file mentions into explicit controls. While a reply streams, it freezes completed blocks and highlights a growing fence from saved Shiki grammar state; the final render uses the same span tree ([incremental renderer](../../../.agents/notes/implemented/architecture/2026-08-06-web-markdown-incremental-ast-renderer.md), [streaming fence highlighting](../../../.agents/notes/implemented/feature/2026-08-20-web-streaming-fence-highlight.md)). `TerminalBlock`, `ReadBlock`, `DiffBlock`, `SearchBlock`, and `WebBlock` render the matching tool-result intent with copy controls, overflow handling, and ANSI processing where applicable. `JsonTree` exposes every visible JSON row, including primitive leaves, through one roving tree-item Tab stop; Arrow keys, Home, End, Enter, and Space follow the tree pattern while the visual chevron stays out of the accessibility tree. `JsonBlock` inspects a JSON value as code, while `MessageText` remains the literal-text primitive for user-authored content.
