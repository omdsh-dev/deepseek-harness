...
`Modal` 提供共用的无障碍对话框约定：通过 `title` 或 `labelledBy` 命名对话框，打开时把焦点移入其中，约束 Tab 与 Shift+Tab 循环，仅允许最上层嵌套对话框响应 Escape 或遮罩激活，在模态栈存在期间把 `#root` 设为 `inert`，并在最后一个对话框关闭后把焦点还给触发控件。`headless` 只改变可见 chrome，不会移除焦点和模态行为。
