...
列出与创建失败会作为当前弹窗内的实时警报出现，而不是另开错误弹窗。输入的路径不可读时，焦点留在路径编辑框，先前列表也会保留以便修正；首次列出主目录失败时，路径编辑框继续作为恢复入口。新建文件夹失败时，焦点留在嵌套弹窗的名称输入框以便重试；关闭嵌套弹窗后，焦点回到**新建文件夹**，父级浏览器保持打开。
...
对话框是 680×500 的 Miller 分栏视图（在较矮或较窄的视口中限制尺寸），经 `ctx.workspaces` 驱动宿主的 `listDirectory` 与 `createDirectory` 原语。两处注册经嵌套的 `ctx.slots.inject()` 调用作为一次事务性效果安装，因为任一声明条目都可能晚些激活或替换其声明；对话框文案注册在本包自己的 locale 命名空间下，让两份字典作为一个单元落地。浏览类失败留在对话框自己的 `role="alert"` 提示区内，因此本填充从不驱动持有方的 `onError` 分支。node 半部是一个空 `apply`，让插件留在宿主名单上。
