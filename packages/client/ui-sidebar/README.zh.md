...
在宽度大于 600px 的视口中实时收起时，展开内容在当前宽度淡出，上方控件共用一次淡入并左移进入 56px 轨道，由布局的栏滑动结束整段动画。视口不超过 600px 或请求减少动态效果时，轨道会在绘制前完成布局，使获得焦点的切换按钮不会停留在原来的展开位置并被裁切。页面初始即为收起状态时会静态渲染轨道。固定在底部的 `sidebar.settings` 控件只共用动画淡入时序，不发生横向位移。
