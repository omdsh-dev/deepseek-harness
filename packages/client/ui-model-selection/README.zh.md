...
composer 模型位是只占一个顺序 Tab 停靠点的菜单按钮。下／上方向键从根菜单首项／末项打开；方向键、Home 与 End 在当前层移动；右方向键进入 Model 或 Effort，左方向键或 Escape 返回其所属根项目。进入子层时，如有已选值则先聚焦该值。关闭根菜单或接受选择后会恢复触发器焦点，除非用户已自行把焦点移到其他位置。
