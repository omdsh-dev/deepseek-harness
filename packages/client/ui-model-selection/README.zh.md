...
与 `ui-conversation` 及命令包一起挂载本插件。composer 显示模型控件，`/model` 则以弹窗打开同一目录。模型菜单公开选择状态，支持方向键、Home 和 End。Tab 或 Shift+Tab 关闭菜单而不选定值，并沿原生焦点顺序移动。Escape 从下钻面板返回父面板，再关闭根菜单并恢复触发器焦点。下钻面板聚焦当前选项，返回时聚焦其父控件。当前模型和推理等级由 Host 提供。
