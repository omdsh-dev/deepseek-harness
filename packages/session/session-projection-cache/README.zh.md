...
缓存是投影注册表检查点接口上的折叠捷径，存于 `per-record` 领域数据表中。它带来七项后果：读取绝不绕过领域写入链；每个会话的检查点提交保持调用顺序；每次后台写入都 fail-soft；`ver` 不匹配时丢弃而不迁移记录；记录必须通过实时单元的 `stateSchema`；写入通过无损 JSON 边界替换一份完整会话记录；日志领先，缓存跟随。
