...
- **Core Web accessibility** (`pnpm run test:web:accessibility`): the diagnostic lane exercises P0 routes in Chromium, Firefox, and WebKit. On a clean commit, `pnpm run test:web:accessibility:evidence` emits exact-revision `dsh-non-at-browser/1.0.0-draft` under `.artifacts/accessibility/`; dirty state or invalid assertions fail, and only three engines yield `pass`. These automated results are non-AT evidence, not proof of real zoom, Windows High Contrast, screen-reader output, or disabled-user completion ([decision](../.agents/notes/implemented/testing/2026-08-31-versioned-core-browser-accessibility-evidence.md)).
