...
- **Coverage gate** (`pnpm run test:coverage`): enforces per-file 100% on `packages/*/*/src`. Treat uncovered code as possibly dead; coverage proves execution, not shipped behavior. Full `packages/shell/pwsh-local/src` coverage needs a real `pwsh`. Without it, executor suites self-skip and `vitest.config.ts` exempts the corresponding source; CI runners with `pwsh` enforce the full threshold. Partitioned runs pin one probe across every shard and the merged report, keeping test selection and exclusions aligned.
