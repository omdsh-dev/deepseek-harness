...
- **核心 Web 无障碍**（`pnpm run test:web:accessibility`）：诊断车道会在 Chromium、Firefox 和 WebKit 中执行 P0 路由。在干净 commit 上，`pnpm run test:web:accessibility:evidence` 会在 `.artifacts/accessibility/` 下生成固定到精确 revision 的 `dsh-non-at-browser/1.0.0-draft`；脏状态或无效断言会失败，只有三引擎才产生 `pass`。这些自动化结果属于非 AT 证据，不能证明真实缩放、Windows 高对比度、读屏输出或残障用户独立完成任务（见[决策记录](../.agents/notes/implemented/testing/2026-08-31-versioned-core-browser-accessibility-evidence.zh.md)）。
