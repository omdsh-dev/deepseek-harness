...
async function tabTo(page: Page, target: Locator, limit = 80): Promise<void> {
  ...
  for (let index = 0; index < limit; index++) {
    ...
    visited.push(await page.evaluate(() => document.activeElement?.textContent?.slice(0, 100) ?? 'none'))
  }
  const focusState = await targetElement.evaluate(element => ({
    target: element.getAttribute('aria-label') ?? element.textContent,
    targetConnected: element.isConnected,
    width: element.getBoundingClientRect().width,
    height: element.getBoundingClientRect().height,
    active: document.activeElement?.getAttribute('aria-label') ?? document.activeElement?.textContent?.slice(0, 100),
  }))
  throw new Error(`${browserName}: keyboard focus did not reach the target after ${String(limit)} Tab presses: `
    + JSON.stringify({ focusState, visited }))
}
...
describe(`assembled core accessibility: ${browserName}`, () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(join(scaffold.workspaceCwd, 'a.txt'), 'alpha accessibility\n')
    await writeFile(join(scaffold.workspaceCwd, 'b.txt'), 'beta accessibility\n')
  ...
  it('publishes named shell landmarks and operates both window splitters without a pointer', async () => {
    ...
    const detailsPane = page.locator('#dsh-rightbar-pane')
    ...
    expect(await page.getByRole('complementary', { name: 'Right sidebar' }).count()).toBe(0)
    ...
    // The right sidebar belongs to the selected Session. Enter one through the
    ...
    await expect.poll(() => collapse.evaluate(element => document.activeElement === element)).toBe(true)
    const details = page.getByRole('separator', { name: 'Right sidebar' })
    await details.waitFor({ timeout: 15_000 })
    expect(await details.getAttribute('aria-controls')).toBe('dsh-rightbar-pane')
    await tabTo(page, details)
    await expectFocused(details)
    await page.keyboard.press('Home')
    await expect.poll(() => details.getAttribute('aria-valuenow')).toBe('300')
    await page.keyboard.press('ArrowLeft')
    await expect.poll(() => details.getAttribute('aria-valuenow')).toBe('310')
    ...
    expect(await page.getByRole('complementary', { name: 'Right sidebar' }).count()).toBe(1)
    await expectFocused(details)
  ...
  it('operates the Workspace tree and search without a pointer', async () => {
    ...
    const searchInput = page.locator('input[aria-label="Search session names"]')
  ...
  it('operates the assembled model menu and command combobox with one current item', async () => {
    ...
    const modelOption = page.getByRole('menuitemradio', { name: 'DeepSeek-V4-Flash', exact: true })
    ...
    const option = listbox.getByRole('option').filter({
      has: page.getByText('DeepSeek-V4-Flash', { exact: true }),
    })
  ...
  it('separates file disclosure and open-file keyboard actions', async () => {
    ...
    await page.keyboard.press('Enter')
    const preview = page.locator('[data-textpreview-state="text"]')
    await preview.waitFor({ timeout: 10_000 })
    await expect.poll(() => preview.locator('[data-textpreview-path]').textContent())
      .toBe(join(scaffold.workspaceCwd, 'a.txt'))
    expect(await disclosure.getAttribute('aria-expanded')).toBe('false')
    const collapse = page.getByRole('button', { name: 'Collapse right sidebar', exact: true })
    await tabTo(page, collapse)
    await page.keyboard.press('Enter')
    await expect.poll(() => page.locator('#dsh-rightbar-pane').getAttribute('aria-hidden')).toBe('true')
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
