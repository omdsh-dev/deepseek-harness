...
describe('web e2e: cold blank Session visibility', () => {
  ...
  it('keeps the verified cold blank Session out of the sidebar', async () => {
    ...
    await list.getByText('No sessions yet', { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.getByRole('tree', { name: 'Sessions' }).count()).toBe(0)
    expect(await list.getByText(WORKSPACE_NAME, { exact: true }).count()).toBe(0)
    const sidebar = await captureStableAria(page, '[data-session-list]', scaffold.workspaceCwd)
