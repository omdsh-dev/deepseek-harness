...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  try {
    ...
    await Promise.all(originalClientArtifacts.map(async ([path, content]) => {
      const destination = join(REPO_ROOT, path)
      await mkdir(dirname(destination), { recursive: true }).catch((error: unknown) => failures.push(error))
      await writeFile(destination, content).catch((error: unknown) => failures.push(error))
    }))
