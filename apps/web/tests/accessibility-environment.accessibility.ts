...
async function focusedGeometry(page: Page): Promise<FocusGeometry> {
  return await page.evaluate(() => {
    ...
    if (!(active instanceof HTMLElement) || active === document.body) {
      return {
        tag: active?.tagName ?? 'none', label: '', hit: null,
...
describe(`assembled accessibility environments: ${browserName}`, () => {
  ...
  it('keeps keyboard focus visible and unobscured in the narrow shell and Settings', async () => {
    onTestFailed(() => saveFailureShot(page, `accessibility-environment-${browserName}-focus`))
    ...
    for (let index = 0; index < navCount; index++) {
      ...
      expect(overflow, `${browserName}: Settings ${JSON.stringify(section?.trim())} must reflow`).toEqual([])
    }
