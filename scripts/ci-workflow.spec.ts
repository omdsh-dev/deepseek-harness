...
describe('CI workflow', () => {
  ...
  it('keeps required Wine and split native Windows jobs with failover, plus a master-only standby', () => {
    ...
    if (!isRecord(workflow.jobs)
      ...
      throw new TypeError('CI workflow must define windows, windows-build, windows-coverage, windows-native-tests, windows-observational, node-24, node-24-coverage, node-24-consumers, node-24-accessibility, node-compat, and all-checks-passed; ci-master must define wine-apt-cache and serial-windows')
    }
    ...
    // Forks preserve that profile while bounding standard four-core runners
    // to the same owner-scoped budget as Linux.
