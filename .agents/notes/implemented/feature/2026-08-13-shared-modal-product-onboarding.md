...
**Both steps share one modal component and one interaction owner.** `OnboardingModal` wraps the existing ui-primitives `Modal` and supplies the common title and content geometry. The shared Modal alone owns `#root` inert, covered-dialog inertness, initial focus, Tab containment, nested dismissal, and connected-opener restoration for exactly the visible lifetime. A nested dialog removes every covered dialog from keyboard and assistive-technology interaction, then restores the next dialog and its connected opener when it closes. The welcome step passes its visible heading as an explicit initial-focus target; the credential step retains its form autofocus. Escape and mask clicks do not silently complete mandatory onboarding; each step exposes only its explicit actions. A step still loading private facts returns `null`, so it paints and blocks nothing.
...
A fresh loopback profile sees the specified internal-testing notice, then an inline DeepSeek key dialog only when no provider is usable. Keyboard users move deterministically from the programmatically focused welcome heading to its sole action in either Tab direction, while focus cannot escape the active dialog. Acknowledgement remains versioned in `settings.yaml`, secrets remain write-only in `.credentials.yaml`, and already-ready or unsupported deployments render no onboarding chrome while readiness loads. The Models package now owns product-onboarding presentation as well as provider configuration; its README and browser coverage make that broader responsibility explicit. This decision restores a concise testing-stage notice after the historical [full-viewport beta notice removal](../../archived/simplification/2026-08-13-remove-first-run-beta-notice.md) without restoring that notice's telemetry copy or takeover layout.
