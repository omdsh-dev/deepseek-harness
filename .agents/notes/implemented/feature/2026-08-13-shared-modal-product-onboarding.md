...
**Both steps share one modal component and one interaction owner.** `OnboardingModal` wraps the existing ui-primitives `Modal` and supplies the common title and content geometry. The shared Modal alone owns `#root` inert, initial focus, Tab containment, nested dismissal, and connected-opener restoration for exactly the visible lifetime. The welcome step passes its visible heading as an explicit initial-focus target; the credential step retains its form autofocus. Escape and mask clicks do not silently complete mandatory onboarding; each step exposes only its explicit actions. A step still loading private facts returns `null`, so it paints and blocks nothing.
