...
The shell renders the conversation column as `main` with one localized level-one application heading, keeps every shell control inside a landmark, renders the Sidebar and Details panels as named complementary landmarks, and exposes both layout separators as named, focusable vertical separators with value metadata and Left/Right plus Home/End keyboard resizing. Empty workspace containers do not claim a `tree` role until tree items exist. The composer always has a localized accessible name and does not advertise popup state that belongs to its sibling workspace dialog. The conversation exposes a named `log` whose streaming mutations are not live, labels user and Assistant articles, and mounts one polite status announcement when a running turn becomes idle; it does not vocalize every token or tool delta.

Visual tokens used for ordinary text meet the applicable contrast threshold in both themes. Plugin configuration uses explicit `list` and `listitem` ownership across slot wrappers instead of invalid native-list children. Whole-row tool disclosures use a native sibling button overlay, so file links and row actions are not nested inside another interactive control. The button receives one explicit, stable spoken name containing the visible title and status or summary, while duplicate decorative icons and visual copy are hidden from the accessibility tree; a real file link remains a separate named control above the overlay. Scrollable Details content is a named, keyboard-focusable region. These structural contracts are owned by the components that emit them and are exercised in the assembled browser, where computed color and final slot-expanded markup are observable.
...
Component tests pin dialog stacking, focus containment, background inertness, return focus (including the image lightbox), menu and typeahead traversal, combobox ownership, tree roving keys and disclosure, radio-group navigation, feedback boundary traversal, timeline range selection, ledger row navigation, collapsed-search accessibility-tree exclusion and search return focus, landmarks, separator metadata and resizing, roving tab lists and tab-panel relationships, disclosure regions, composer names, message-log labels, article labels, and the completion transition. The accessibility workflow runs the GUI suite on Linux, macOS, and Windows, then typechecks, lints, builds the official distribution, and replays the assembled browser suite on a sandbox-capable Linux runner.

The product-level axe lane scans the real assembled Chromium application with no disabled rules. It covers the empty shell, every Settings section, both plugin tabs, a settled seeded conversation, expanded tool output, open menus, and both themes. On macOS 15.5, a VoiceOver-enabled Safari 18.5 run additionally confirmed the native accessibility tree and the sequential focus route from the navigation controls through workspace and composer controls; keyboard opening Settings and Escape restoration returned focus to its trigger. The automation surface cannot reliably capture the audio utterance or VoiceOver cursor itself, so listener-verified spoken output remains a separate manual record rather than an inferred pass. The optional `dsh-accessibility` companion audits 17 structural contracts and provides operating guidance; a passing audit is a regression signal, not assistive-technology certification.
