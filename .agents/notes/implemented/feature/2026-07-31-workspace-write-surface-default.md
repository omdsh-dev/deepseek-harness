...
A genuinely fresh session pins `permission/preset: workspace-write`, `sandbox/mode: workspace-write`, and `approval/policy: ask` before execution. Existing and resumed sessions retain their logged permission, and changing the General-settings default affects only sessions created afterward. The browser keeps its Access picker, answerable approval cards, and risk confirmation for Full access. That shared confirmation associates its risk text with the dialog, hides the warning glyph from the accessibility tree, starts on the acknowledgement checkbox, and returns focus to the durable Access or Settings trigger after cancellation or a completed asynchronous change. The TUI gains the existing `/permission` command because the shared Permission service activates its command child there.
...
The keyless shipped-TUI pseudo-terminal smoke boots the real Loader tree, reads the persisted first request, and asserts both the `sandbox_permissions`/`justification` bash schema and the initial workspace-write event triplet. The shipped-Web composition smoke asserts the same policy, approval, and Permission defaults. Component tests pin the dialog description, decorative glyph, initial focus, disabled confirmation, and focus restoration across cancellation, asynchronous locking, unavailable owners, and unmount. Assembled Chromium scenarios prove both the current-session Access picker and future-session Settings selector through their real command/RPC paths; the accessibility-tree golden retains the risk copy without exposing the warning glyph as an unnamed image.
