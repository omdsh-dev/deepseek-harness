...
`windows-build` 与 `windows-native-tests` 是 `all checks passed` 的依赖项；其工作区构建和定向原生进程结果具有阻断性。worker-thread 生命周期文件会按已有的“脚本执行”“生命周期”“worker 死亡”“服务 API”四个命名套件，分别在单文件、单 worker 的 Vitest 进程中运行，再运行其余原生 spec。第一次隔离源于标准 `windows-2025` fork 运行器在另外 4 个文件全部通过时丢失了生命周期文件；之后一个干净运行器仍在 54 个用例通过 35 个后丢失 fork，证明仅做文件级隔离仍会让一个长生命周期 fork 累积过多有意终止的 worker。套件级进程隔离保留全部断言，同时约束原生 Worker 累积；两次失败运行都保留记录，且绝不用 retry 掩盖。`windows-coverage` 仍是常规作业，但不在聚合流程的 `needs` 中，因此逐文件 100% 覆盖率结果会保持红灯并可见，却不会延迟必需判定。`windows-observational` 同样不在聚合流程的 `needs` 中，并使用 `continue-on-error`，因为静态检查、文档、包与构建产物的阻断性判定由 Linux 负责。
