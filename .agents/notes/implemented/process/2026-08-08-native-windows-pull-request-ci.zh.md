...
`windows-build` 与 `windows-native-tests` 是 `all checks passed` 的依赖项；其工作区构建和定向原生进程结果具有阻断性。worker-thread 生命周期套件会先在独立的单文件、单 worker Vitest 调用中运行，再运行其余原生 spec。此前一次标准 `windows-2025` fork 运行器在另外 4 个文件全部通过时丢失了该文件的 fork；这项隔离会把套件中有意构造的 worker 终止与崩溃探针留在独立 Vitest 进程中，失败运行仍保留记录，也不会用 retry 掩盖。`windows-coverage` 仍是常规作业，但不在聚合流程的 `needs` 中，因此逐文件 100% 覆盖率结果会保持红灯并可见，却不会延迟必需判定。`windows-observational` 同样不在聚合流程的 `needs` 中，并使用 `continue-on-error`，因为静态检查、文档、包与构建产物的阻断性判定由 Linux 负责。
