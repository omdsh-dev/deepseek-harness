...
下一轮 Linux coverage 已将失败范围收敛为两个边界缺陷，而不是大范围资源饥饿。SQL ownership gate 拒绝了一个把变量 SQL 字符串继续传给父类的回归测试 override；现在改为在数据库构造函数中推进模拟的 journal 前耗时，不再创建无归属查询源。更重要的是，真实 PowerShell 场景证明：当 host 与 PSReadLine 仍在协商终端协议时写入 setup，启动结果可能返回默认 `PS …>` 提示符，也可能只反复重绘 setup 源码而不执行。PowerShell 启动因此被明确拆成两个阶段：DSH 先等待原生提示符并排空自动生成的终端响应，然后才通过“必须出现受控提示符”的 send 提交 UTF-8 与受控提示符 bootstrap。第二阶段禁用静默和通用 stdin-wait 回退，只有同时观察到 DSH 私有 OSC 提示符标记与前台 shell 证据才会完成。SQLite、SQL 边界、终端启动与就绪套件合计 158 项测试全部通过；持久化 PowerShell 状态与 secret 清理场景在真实 PowerShell 7.6.5 上连续 10 次通过，完整真实 PowerShell 测试组中的状态与 UTF-8 两个场景也全部通过。

由此触发的完整矩阵已经通过 Windows 100% coverage、snapshot 与 artifact 门禁、Windows native 测试、Wine 门禁、Node 22 与 26 兼容性，以及四个平台的 release-shaped runtime。Linux 同样报告 statement、branch、function 与 line 四项覆盖率均为 100%，但一个 Agent Team 恢复测试收到契约允许的 `queued`，而测试只接受同样合法的即时 `accepted`。该恢复测试及其 FIFO 相邻场景现在会等待 durable mailbox 清空，并在恢复后的子会话日志中检查精确 message id；JSONL 与 SQLite 持久化套件已连续运行 15 轮，共 150 项测试全部通过。Static 与 Windows observational 通道除这一点外分别通过 42/43 与 49/50 项检查；两者唯一共同失败都是新增公开 readiness 参数缺少 `@param`，现已由 export-JSDoc 门禁覆盖。最终一轮新矩阵仍将作为这两项测试契约修复与聚合结论的集成检查。
