...
The workflow-worker-thread owner is one explicit process-bound exception inside that same instrumented gate. The ordinary shard role excludes `workflow-worker-thread.spec.ts`; after all shards settle, the coordinator runs its four exact top-level suites serially in four fresh single-worker Vitest processes. Each suite remains instrumented and writes its own blob. This is a process boundary, not a coverage exemption: the four slices collectively execute all 54 tests and contribute their coverage to the final report. The boundary mirrors native Windows CI, where an entire 54-test fork had twice disappeared without an assertion result even after the rest of the native inventory was separated.

The coordinator waits for every shard and isolated suite, validates that the blob directory contains exactly the expected files, and then runs one `vitest --merge-reports ... --coverage` command. Only that merged command applies the repository's per-file statement, branch, function, and line thresholds, so a child is never judged against an intentionally partial inventory.

Before it constructs any child command, the coordinator probes PowerShell availability once through `pinPwshTestAvailability()` and writes the resulting `DSH_TEST_PWSH_AVAILABLE=0|1` fact into every partition and the merge command. `vitest.config.ts` and every real-PowerShell suite consume that inherited fact; a non-partitioned Vitest invocation pins the same fact while evaluating its config before workers start. Invalid inherited values fail configuration. This makes the pwsh-less source exclusions and the suites that need `pwsh` one run-wide decision even if executable startup availability changes before the merged report evaluates the config again.
...
Shard and isolated-suite children stream stdout and stderr through the coordinator. The coverage gate opts into `run-gates` streaming, so test progress and failures reach CI logs as they occur without buffering the complete log in the scheduler. The coordinator also retains a bounded 64 KiB combined tail per child; when a child settles unsuccessfully, it prints the spawn error, exit code, or signal and repeats that tail before validating the complete blob set, keeping the specific Vitest failure beside the final child diagnostic.
...
`scripts/coverage-partitions.spec.ts` pins argument construction, package-script separator removal, one-worker shards, the four serial isolated workflow-worker commands and their exact suite names, one inherited PowerShell fact across every child and merge, the single merged threshold command, failed-test merging, failure diagnostics before complete-blob validation, waiting for sibling partitions after a spawn failure, and link-safe cleanup. `scripts/pwsh-test-availability.spec.ts` pins inherited true/false decisions, one-time probe publication, reuse, and invalid-value rejection. `scripts/run-gates.spec.ts` pins opt-in selection, invalid-count rejection, both native Windows coverage gates' complete-build dependency, the complete Windows inventory with its blocking split, and unbuffered streamed output. React fake-timer cases that can move between partitions advance timers inside `act()`; geometry-dependent portal tests stub their element rectangles so a different shard schedule cannot turn deferred updates or jsdom coordinates into coverage-only failures.

The triggering Windows coverage run retained 479 passed files, one skipped file, 7,419 passed tests, and 15 skipped tests in shard 1 before Vitest reported one fork exiting unexpectedly. Shard 2 completed 479 files and 7,891 tests. Comparing the completed-file inventory with the preceding successful run identified exactly one absent owner: `workflow-worker-thread.spec.ts`, whose preceding result contained all 54 tests. No assertion failed and the job was not rerun. Local replay of the new four-process boundary passed 22, 21, 7, and 4 selected tests, covering all 54 unique tests; coordinator, gate, and workflow contracts passed 106 tests before the next integrated run. The coordinator contract reads the owner file and requires the selector roster to equal every direct child suite, so a renamed or added suite cannot silently become an all-skipped slice.
...
**Keep all 54 workflow-worker tests in one coverage fork.** Rejected because the fork can disappear after running almost the entire file, producing neither a file result nor attributable assertion. Four suite-level processes preserve the exact test and coverage inventory while making one process failure attributable to a bounded slice.
...
**Probe PowerShell independently in each partition and merge process.** Rejected because executable startup is an observed runtime fact, not an immutable runner property. Independent probes can skip every real-PowerShell test and later remove the matching source exclusion before threshold evaluation.

## Consequences

Coverage pays one Vitest startup/configuration cost per partition, four short serial workflow-worker startup costs, and one report-merge cost, but it avoids another workflow topology and keeps one final threshold verdict. Partition output may interleave; isolated suite labels and Vitest file identities retain exact attribution.

Linux and Windows use the same coordinator with platform-specific partition counts and surrounding worker budgets. Native Windows starts both coverage gates after the complete build because its instrumented corpus can consume built artifacts; Linux's dedicated coverage job does not share a workspace with a concurrent build. Local coverage stays simple unless a caller explicitly chooses the partitioned package script and supplies a valid count greater than one. A run keeps its first PowerShell result even if a later probe would differ; consistency between executed tests and evaluated exclusions takes precedence over discovering a mid-run availability change.
