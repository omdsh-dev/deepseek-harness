...
CI's `scripts/run-web-snapshots.ts` first runs `hmr-live.e2e.ts`, `cordis-tool-round.e2e.ts`, `workflow-run.e2e.ts`, and `workspace-management.e2e.ts` as separate serial Vitest invocations. The HMR scenario mutates built workspace state, while the other three own long-lived, stateful browser/host sequences: Cordis approval and steering, a real workflow worker plus local child navigation, and one shared workspace-management page across pointer, menu, archive, reload, and directory operations. Fresh processes keep those owners out of a resource-contended file pool without changing their waits, assertions, or goldens. After all four pass, one bounded Vitest pool runs every remaining file. The remote-authority lane explicitly maps its browser hostname to loopback and makes the scaffold token exchange over loopback with the same HTTP Host header, so Host trust remains covered without depending on an operating system's wildcard `.localhost` resolver. Every child inherits stdio, and the enclosing gate streams that output through `run-gates`.
...
**Run every browser file inside the parallel pool.** Rejected because HMR mutates shared built state, while Cordis, workflow-run, and workspace-management retain lifecycle state across multi-step scenarios. A bounded pool remains appropriate for the independent files; the four explicit owners receive fresh processes so pool contention cannot invalidate their stateful sequences.
...
Before merge, every PR proves that the current web assembly matches all committed browser expected outputs; a missing refresh fails in the same PR that changes the assembly. The cost is Chromium provisioning, four serial files, and one bounded pool in the consumer job; the consumer-owned build and browser cache avoid duplicate builds and downloads. Parallel-file failures stream immediately, but a worker-budget or ownership-boundary change still requires completed end-to-end evidence rather than an elapsed-time guess. The gate makes no claim of cross-platform browser consistency, and if a Playwright/Chromium upgrade changes the ARIA format, the upgrade PR must explicitly refresh the expected outputs and review the churn.
